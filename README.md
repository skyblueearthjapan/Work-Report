...
4. 責任者：確認印（未押印はクローズ不可。印刷・PDF保存は押印前でも可）
...
- 確認印（電子印）が押されるまで**クローズ**不可（サーバーでも検証）。**印刷・PDF保存は押印前でも可**（現場でお客様控えを出すため。確認印欄は空欄で出力される）。
- 印刷は必ず **A4 1枚**に収まる（`doPrint`が印刷可能高さ 1039px を超えた分だけ自動縮小。拡大はしない）。
